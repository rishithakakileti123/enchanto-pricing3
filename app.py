...
st.title("🧠 AI Agent for Dynamic Pricing Optimization")
...
def ai_pricing_agent(row):
    ...
    if inventory < 20 and demand > 7:
        suggested *= 1.20
        reasoning.append("🔥 Scarcity Surge (+20%)")

    elif inventory > 100 and demand < 4:
        suggested *= 0.90
        reasoning.append("🏷️ Clearance Move (-10%)")

    if suggested > competitor * 1.15:
        suggested = competitor * 1.10
        reasoning.append("⚔️ Competitor Match")

    min_price = cost * 1.20
    if suggested < min_price:
        suggested = min_price
        reasoning.append("🛡️ Margin Protection")
...
category = st.sidebar.selectbox("📂 Category", sorted(product_data["Category"].unique()))
filtered = product_data[product_data["Category"] == category]

product_name = st.sidebar.selectbox("🧴 Product", filtered["Product Name"].unique())
...
# 2️⃣ PROFITABILITY ANALYSIS (Stacked)
with col2:
...
# 3️⃣ COMPETITOR PRICE TREND (Line Chart)
fig3 = px.line(
    history,
    x="Date",
    y="Competitor_Price",
